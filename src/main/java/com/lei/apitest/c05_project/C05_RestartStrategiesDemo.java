...
public class C05_RestartStrategiesDemo {
    public static void main(String[] args) throws Exception {
        ...
        // 只有开启了checkpoint 才会有重启策略  // 默认保存到内存中，即检查点保存在JobManager内存中，状态保存在TaskManager内存中
        ...
        // 设置固定延迟固定次数重启，重启3次，每次重启间隔2秒
        env.getConfig().setRestartStrategy(RestartStrategies.fixedDelayRestart(3, 2000));
        // 强烈建议使用异步快照来防止数据流阻塞
        StateBackend memoryStateBackend = new MemoryStateBackend(MemoryStateBackend.DEFAULT_MAX_STATE_SIZE, true);
